...
extern unsigned char far * pascal _argv[256];
extern unsigned int pascal _psp;
extern unsigned int pascal _startip;
extern unsigned int pascal _startcs;
...
extern unsigned int far pascal getvideomode(void);
...
extern void far pascal inportsb(unsigned int, void far *, unsigned int);
extern void far pascal inportsw(unsigned int, void far *, unsigned int);
extern void far pascal outportsb(unsigned int, void far *, unsigned int);
extern void far pascal outportsw(unsigned int, void far *, unsigned int);
extern unsigned int far pascal wherex(void);
extern unsigned int far pascal wherey(void);

unsigned char far * pascal prevdir="..";
unsigned char far * pascal wild_all="*.*";
unsigned char far * pascal rootdir="\\";
t_heap far * pascal HEAP_head;
t_heap far * pascal HEAP_tail;
unsigned int pascal scanning_level;

void far pascal (far pascal *putpixel)(unsigned int, unsigned int, unsigned int)=_putpixel12h;
...
void far * far pascal ltoa(void far *s_ptr, unsigned long s_num, unsigned int s_radix, unsigned int s_sign)
{
 unsigned char far *s_sptr=s_ptr; unsigned char s_str[20]; unsigned int s_count=0U;
	   if(s_sign){if(s_num&0x8000){*(s_sptr++)='-'; s_num=(~s_num)+1U;}}
	   do{s_str[s_count++]=_num[(unsigned int)(s_num%s_radix)]; s_num/=s_radix;}while(s_num);
	   while(s_count)*(s_sptr++)=s_str[--s_count];
	   *(s_sptr)='\0'; return(s_ptr);
}
...
unsigned long far pascal atol(void far *s_str, unsigned int s_radix)
{
 unsigned char far *s_string=s_str; unsigned long s_ret=0U, s_num=1U; unsigned int s_len;
	   if(*(s_string)=='-')s_string++;
	   s_len=strlen(s_string); if(s_len==0U)return(0U);
	   while(s_len){s_len--; s_ret+=s_num*tonum(*(s_string+s_len)); s_num*=s_radix;}
	   if(*((unsigned char far *)s_str)=='-')s_ret=(~s_ret)+1U;
	     ...
	     if(s_fmtchar)
	     {
	      ...
	      switch(s_fmtchar)
	      {
	       case 'D': case 'U': case 'I': case 'X': case 'O': case 'B':
		    if(s_long)s_data=(unsigned long)*(((unsigned long far *)s_varg)++);
		    ...
		    if(s_long)ltoa(&s_numbuff[0], s_data, s_radix, s_sign);
...
void far pascal _initmain(void)
{
 unsigned int s_count;
	   HEAP_head=HEAP_tail=0LU; _argc=1U;
	   for(s_count=0U;s_count<256U;s_count++)_argv[s_count]=MZ_NULL;
